# Ghostwood Municipal Airport - Playdate Game (Claude Code Prompt)

## Project overview

Build a Lua game for the Playdate handheld console using the Playdate SDK. The game is called **Tower** — a card-based air traffic control game set at Ghostwood Municipal Airport, a small remote airstrip in the Pacific Northwest with a Twin Peaks-inspired aesthetic (folksy but weird).
...
Build incrementally. Get each milestone running on hardware before moving to the next.
...
- Build: `pdc source/ builds/tower.pdx` (Playdate compiler)
- Run simulator: `open builds/tower.pdx` (opens in Playdate Simulator)
